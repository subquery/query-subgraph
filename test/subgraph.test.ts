...
describe('subgraph plugin test', () => {
  ...
  async function initDatabase() {
    ...
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."accounts"."id" IS 'id field is always required and must look like this';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."accounts"."public_key" IS 'The public key of this account (same across all Polkadot parachains)';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."accounts"."first_transfer_block" IS 'The first block on which we see a transfer involving this account';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."accounts"."last_transfer_block" IS 'The most recent block on which we see a transfer involving this account';`
    );
    ...
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."transfers"."id" IS 'id field is always required and must look like this';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."transfers"."amount" IS 'Amount that is transferred';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."transfers"."block_number" IS 'The block height of the transfer';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."transfers"."date" IS 'The date of the transfer';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."transfers"."from_id" IS 'The account that transfers are made from';`
    );
    await pool.query(
      `COMMENT ON COLUMN "${dbSchema}"."transfers"."to_id" IS 'The account that transfers are made to';`
    );
  ...
  async function insertMetadata(key: string, value: string) {
    await pool.query(
      `INSERT INTO ${dbSchema}._metadata(key, value, "createdAt", "updatedAt") VALUES ('${key}', '${value}', '2021-11-07 07:02:31.768+00', '2021-11-07 07:02:31.768+00');`
    );
  }
    ...
    it('Not equal', async () => {
      const results = await graphqlQuery(gql`
        query MyQuery {
          accounts(
            first: 10
            where: { id_not: "14tkt6bunjkjdfyqvdnfbqzdawmj7waqwfumxmizjhhrr1gs" }
          ) {
            id
          }
        }
    ...
    it('Contains nocase', async () => {
      const results = await graphqlQuery(gql`
        query MyQuery {
          accounts(
            first: 100
            where: { id_contains_nocase: "njkjdfyqvdnfbqzdawmj7waqwfumxmizJHHr" }
          ) {
            id
          }
        }
    ...
    it('Not contains nocase', async () => {
      const results = await graphqlQuery(gql`
        query MyQuery {
          accounts(
            first: 100
            where: { id_not_contains_nocase: "njkjdfyqvdnfbqzdawmj7waqwfumxmizJHHr" }
          ) {
            id
          }
        }
    ...
    it('not_ends_with', async () => {
      const results = await graphqlQuery(gql`
        query MyQuery {
          accounts(
            first: 100
            where: { id_not_ends_with: "dfyqvdnfbqzdawmj7waqwfumxmizjhhrr1gs" }
          ) {
            id
          }
        }
    ...
    it('ends_with_nocase', async () => {
      const results = await graphqlQuery(gql`
        query MyQuery {
          accounts(
            first: 100
            where: { id_ends_with_nocase: "dfyqvdnfbqzdawmj7waqwfumxmizjhhrr1gs" }
          ) {
            id
          }
        }
      ...
      const results2 = await graphqlQuery(gql`
        query MyQuery {
          accounts(
            first: 100
            where: { id_ends_with_nocase: "dfyqvdnfbqzdawmj7waqwfumxmizjhhrr1g" }
          ) {
            id
          }
        }
    ...
    it('not_ends_with_nocase', async () => {
      const results = await graphqlQuery(gql`
        query MyQuery {
          accounts(
            first: 100
            where: { id_not_ends_with_nocase: "dfyqvdnfbqzdawmj7waqwfumxmizjhhrr1gs" }
          ) {
            id
          }
        }
  ...
  describe('order plugin', () => {
    it('order by firstTransferBlock asc', async () => {
      ...
      for (let i = 0; i < fetchedMeta.accounts.length - 1; i++) {
        expect(fetchedMeta.accounts[i].firstTransferBlock).toBeLessThanOrEqual(
          fetchedMeta.accounts[i + 1].firstTransferBlock
        );
      }
    });
    ...
    it('order by firstTransferBlock desc', async () => {
      ...
      for (let i = 0; i < fetchedMeta.accounts.length - 1; i++) {
        expect(fetchedMeta.accounts[i].firstTransferBlock).toBeGreaterThanOrEqual(
          fetchedMeta.accounts[i + 1].firstTransferBlock
        );
      }
    });
